...
module.exports = function (t, a) {
	var el1, el2, el3, node, c1, c2, attr;

	if (typeof document === 'undefined') return;

	el1 = document.createElement('p');
	el2 = document.createElement('div');
	attr = document.createAttribute('someattr');
	attr.value = 'lorem';

	node = t.call(el2, el1, attr, 'Test').childNodes[1];
	a(node.nodeType, 3, "String to Text node");
	a(node.data, 'Test', "String to Text node: content");
	a.deep(toArray(el2.childNodes), [el1, node],
		"Children");
	a(el2.getAttribute('someattr'), 'lorem', "Attr node");
